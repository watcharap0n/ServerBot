...
async def check_quick_reply_duplicate(quick_reply: QuickReply):
    items = await db.find(
        ...
        query={"access_token": quick_reply.access_token}
    )
    items = list(items)
    for item in items:
        if item["name"] == quick_reply.name:
            raise HTTPException(
...
async def update_query_quick_reply(
    ...
    if (await db.update_one(collection=collection, query=query, values=values)) == 0:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail=f"Quick_Reply not found {id}"
        )
    return payload
